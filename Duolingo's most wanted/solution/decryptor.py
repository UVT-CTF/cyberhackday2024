...
def decrypt(block):
    ...
    for char in block:
        ...
        for i in range(5):
            for j in range(7):
                if seion[i][j] == char:
                    decrypted_block += alphabet_table[i][j]
                    found = True
                    break
            if found:
                break
...
def main():
    ...
    with open("encrypted_message.txt", "r", encoding="utf-8") as input_file:
    ...
    decrypted_text = ""
    for block in blocks:
        decrypted_text += decrypt(block)
        global R1, R2
        R1, R2 = Rgen()
    
    with open("decrypted_message.txt", "w", encoding="utf-8") as output:
